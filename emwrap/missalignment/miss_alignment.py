...
from emtools.metadata import Imod, RelionStar, StarFile, Table
...
class MissAlignment(WarpBasePipeline):
    ...
    def _run_relion_warp_convert_import(self, batch):
        ...
        stack_pixel_size = self._relion_import_stack_pixel_size()
    ...
    def _get_stack_pixel_size(self):
        """Return the 'Prepare stacks (A)' value shared by both backend commands."""
        value = self._args.get('prepare_stacks_pixel_size', '')

        if not str(value).strip():
            return None

        stack_pixel_size = float(value)

        if stack_pixel_size <= 0:
            raise ValueError(
                'prepare_stacks_pixel_size must be a positive number, '
                f'received: {value!r}'
            )

        return stack_pixel_size

    def _relion_import_stack_pixel_size(self):
        """Return the required stack pixel size for RELION-only conversion."""
        stack_pixel_size = self._get_stack_pixel_size()

        if stack_pixel_size is None:
            raise ValueError(
                'RELION-only MissAlignment input requires "Prepare stacks '
                '(A)". It is passed as --stack-pixel-size to '
                'relion-warp-convert import, which replaces '
                '--prepare-stacks for RELION-only input.'
            )
    ...
    def _resolve_relion_image_path(self, image_reference):
        ...
        RELION commonly stores paths relative to the project root, while this
        wrapper runs from its own job directory.  Try the current directory
        and every ancestor of the input STAR so both project-root-relative and
        STAR-relative paths work.
        """
        image_path = str(image_reference).split('@', 1)[-1]
        if os.path.isabs(image_path):
            candidates = [image_path]
        else:
            candidates = [os.path.abspath(image_path)]
            directory = os.path.abspath(os.path.dirname(self.inputTs))
            while True:
                candidates.append(os.path.join(directory, image_path))
        ...
        raise FileNotFoundError(
            'Cannot find the tilt-series image referenced by '
    ...
    def _common_missalign_args(self, converted_from_relion):
        ...
        if converted_from_relion:
            ...
            # its XMLs have no MoviePath for --prepare-stacks to use.
            if common_args.pop('--prepare-stacks', None) is not None:
                self.log(
                    'Ignoring --prepare-stacks for RELION-only input; '
                    'relion-warp-convert import created the rescaled stacks.'
                )
        else:
            stack_pixel_size = self._get_stack_pixel_size()
            if stack_pixel_size is not None:
                common_args.update({'--prepare-stacks': stack_pixel_size})
    ...
    def _correct_warp_converted_tilt_series_stars(self, batch):
        ...
        individual_stars = sorted(glob(os.path.join(tilt_series_directory, '*.star')))

        if not individual_stars:
            raise FileNotFoundError(
                'No individual tilt-series STAR files found in: '
                f'{tilt_series_directory}'
            )

        for star_file in individual_stars:
            table_name = os.path.splitext(os.path.basename(star_file))[0]
            table = StarFile.getTableFromFile(table_name, star_file)

            if table is None or len(table) == 0:
                raise ValueError(
                    f'Individual tilt-series STAR is empty or missing its '
                    f'table for {table_name}: {star_file}'
                )

            if 'rlnTomoYTilt' not in table.getColumnNames():
                raise ValueError(
                    f'{table_name}: individual tilt-series STAR is missing '
                    'rlnTomoYTilt'
                )

            corrected_table = Table(table.getColumnNames())
            for row in table:
                row_dict = row._asdict()
                row_dict['rlnTomoYTilt'] = -float(row_dict['rlnTomoYTilt'])
                corrected_table.addRowValues(**row_dict)

            self.write_ts_table(table_name, corrected_table, star_file)

        self.log(
            f'Corrected rlnTomoYTilt in {len(individual_stars)} '
            'Warp-converted tilt-series STAR file(s).'
        )

    def _run_relion_warp_convert_export(self, batch, run_directory):
        """Export MissAlignment XML results using relion-warp-convert."""
        output_star = batch.join(self.OUTPUT_STAR)

        args = Args({
            'relion-warp-convert': '',
            'export': '',
            '--run-directory': os.path.abspath(run_directory),
            '--input-star': os.path.abspath(self.inputTs),
            '--output-star': os.path.abspath(output_star),
        })

        # OPTIONAL
        # Local alignment refinement is only available if the Miss-Alignment model was trained with spline iterations.
        # if 0 < int(self._args.get('infer.yml.iterations_spline', 0)):
        #     args.update({
        #         '--local-motion-directory': batch.join('local_motion')
        #     })    

        self.log(
            'Running relion-warp-convert export: '
            f'run_directory={run_directory}, '
            f'input_star={self.inputTs}, '
            f'output_star={output_star}'
        )

        self.batch_execute(
            'relion_warp_convert_export',
            batch,
            args,
            launcher=self._get_launcher(),
        )

        self._correct_warp_converted_tilt_series_stars(batch)

        if not os.path.isfile(output_star):
            raise FileNotFoundError(
                'relion-warp-convert export finished but did not create: '
                f'{output_star}'
            )
