...
class EMhubTomo:
    ...
    @classmethod
    def _get_home_dir(cls):
        """ Return the installation folder, the one holding the 'emh-tomo'
        script together with 'bashrc', 'emwrap.bashrc' and 'scripts'.

        The generated 'emh-tomo' script exports EMHUB_TOMO_HOME with its own
        location. Installations generated before that variable existed are
        resolved from EMSTACK_HOME ('<installation folder>/source'), which
        'bashrc' exports in both cases.
        """
        if home_dir := os.environ.get('EMHUB_TOMO_HOME'):
            return os.path.abspath(home_dir)

        emstack_home = os.environ.get('EMSTACK_HOME')
        if not emstack_home:
            raise Exception(
                "Neither EMHUB_TOMO_HOME nor EMSTACK_HOME is set, so the "
                "installation folder can not be resolved. Run the 'emh-tomo' "
                "script generated by the installer, which activates the "
                "environment before calling this module.")

        return os.path.dirname(os.path.abspath(emstack_home))
    ...
    def _update_config(cls):
        ...
        overwritten:
          - copy 'emwrap.bashrc' into the installation folder
          - create its 'scripts' folder (if missing) and copy into it
            any missing script template

        Everything goes next to the 'emh-tomo' script, not into the current
        directory: 'emh-tomo' sources 'emwrap.bashrc' from its own folder,
        and 'emwrap.bashrc' in turn resolves the launchers from the
        'scripts' folder next to it.
        ...
        home_dir = cls._get_home_dir()
        if not os.path.isdir(home_dir):
            raise Exception(f"Installation folder not found: {home_dir}")

        print(f">>> Updating configuration files in: {Color.bold(home_dir)}")
        config_templates = cls._copy_missing_templates(config_dir, home_dir)
        ...
        target_dir = os.path.join(home_dir, 'scripts')
    ...
    def main(cls):
        ...
        g.add_argument('--update', '-u', action='store_true',
                       help="Pull the latest changes (git pull --prune) for "
                            ...
                            "(emwrap.bashrc) into the installation folder "
                            "(the one holding this emh-tomo script) and "
                            "create its 'scripts' folder if missing, copying "
                            "into it any missing script template.")
